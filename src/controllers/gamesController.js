...
export const eliminateGame = async (req, res) => {
    ...
    if (!admin) return res.status(403).json({ msg: "The user doesn't have the right permissions to delete a game" });

    try {
        const foundGame = await Game.findById(id);

        if (!foundGame) return res.status(404).json({ msg: "The game ID is not associated with an existing game" });
